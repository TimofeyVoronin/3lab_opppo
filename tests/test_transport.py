...
# ==================================================
# ТЕСТЫ НА ОШИБКИ ВВОДА ДАННЫХ
# ==================================================

def test_create_with_missing_parameters():
    """
    Проверка создания объекта с недостаточным количеством параметров.
    Ожидается возникновение ошибки TypeError.
    """
    with pytest.raises(TypeError):
        # Не хватает параметров (например, отсутствует capacity)
        Plane("Alex", 600, 1500, 3000)


def test_create_with_invalid_data_type():
    """
    Проверка создания объекта с некорректным типом данных.
    Например, строка вместо числа.
    """
    with pytest.raises(TypeError):
        # Передаём строку вместо числа
        Plane("Alex", "fast", 1500, 3000, 8000)
